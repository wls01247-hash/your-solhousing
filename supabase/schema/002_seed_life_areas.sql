-- =====================================================================
-- 시드 데이터: 심리테스트 유형 → 생활권 → 역명
-- 외부 Supabase SQL Editor 에서 001_schema.sql 실행 후에 실행하세요.
-- 멱등 (여러 번 실행해도 안전).
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1) life_area_types (8개 유형)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2) life_areas + life_area_stations
--    DO 블록으로 (유형, 생활권명, 역명배열) 을 일괄 삽입
-- ---------------------------------------------------------------------
do $$
declare
  v_area_id  uuid;
  ...
  s_order    integer;
begin
  -- (type_slug, area_name_ko, display_order, stations[])
  for rec in
    select * from (values
      -- 🚇 환승 알레르기형
...
-- 확인 쿼리 (선택 실행)
-- ---------------------------------------------------------------------
-- select t.slug, t.name_ko, la.name_ko as area, s.station_name_ja
-- from public.life_area_types t
-- join public.life_areas la on la.type_slug = t.slug
-- join public.life_area_stations s on s.life_area_id = la.id
-- order by t.display_order, la.display_order, s.display_order;